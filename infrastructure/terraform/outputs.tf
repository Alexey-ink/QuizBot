output "server_ip" {
  ...
  value = (
    trimspace(var.existing_server_id) != "" ? (
      trimspace(var.existing_server_ip) != "" ?
      var.existing_server_ip :
      data.openstack_compute_instance_v2.existing_server[0].access_ip_v4
...
output "server_name" {
  value = (
    trimspace(var.existing_server_id) != "" ?
    data.openstack_compute_instance_v2.existing_server[0].name :
    openstack_compute_instance_v2.app_server[0].name
  )
}
