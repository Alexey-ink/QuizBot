...
public class ListTagsCommandHandler implements CommandHandler {
    ...
    public ListTagsCommandHandler(TagRepository tagRepository, UserService userService) {
    ...
    public void handle(Update update, AbsSender sender) {
        ...
        try {
            ...
            for (Tag tag : tags) {
                response.append("• ").append(tag.getName()).append("\n");
            }
            
            response.append("\n💡 **Использование:**\n" +
                "• `/show_questions_by_tag <тег>` - просмотр вопросов\n" +
                "• `/delete_tag <тег>` - удаление тега (⚠️ удаляет вопросы без других тегов)");
