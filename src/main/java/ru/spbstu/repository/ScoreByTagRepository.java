...
public interface ScoreByTagRepository extends JpaRepository<ScoreByTag, Long> {
    ...
    @Query("SELECT s.score FROM ScoreByTag s WHERE s.user.telegramId = :userId AND LOWER(s.tag.name) = LOWER(:tagName)")
    Optional<Integer> findScoreByUserTelegramIdAndTagName(@Param("userId") Long userId, @Param("tagName") String tagName);
}
